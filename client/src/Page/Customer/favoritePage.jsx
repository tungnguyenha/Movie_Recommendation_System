...
function FavoritePage() {
    ...
    useEffect(() =>{
      ...
      axios.get(import.meta.env.VITE_GET_FAVORITE + user.userId)
      .then(result => setImage(result.data))
      .catch(err => console.log('errFavorite: ',err));
    },[user.userId])

    const handleRemoveFavorite = (movieId) =>{
      axios.delete(import.meta.env.VITE_DELETE_FAVORITE+ '?MovieId='+movieId+'&userId='+user.userId)
      .then(result => {
    ...
    const handleRemoveAllFavorite = () =>{
      axios.delete(import.meta.env.VITE_DELETE_ALL_FAVORITE+user.userId)
      .then(() => {
        setImage([]);
        notify('Remove success')})
      .catch(err => console.log('remove failed: ', err));
